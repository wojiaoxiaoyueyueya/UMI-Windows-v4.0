...
`pose_data/trajectory.csv` 中的视觉惯性位姿是相对观测量，可作为研究或状态特征使用，但不得冒充机器人控制器实际收到的 `action`。`cooperative_constraint=1` 只表示双手视觉与 IMU 共同确认静止并执行了零速度更新，不代表两手共享绝对世界坐标。未完成相机内参、相机-IMU 外参和统一世界坐标标定时，也不得把该轨迹标注为绝对真值位姿。
