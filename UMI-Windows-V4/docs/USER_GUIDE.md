...
本文面向第一次接触本项目的使用者，也适用于需要换电脑部署、配置设备、录制数据、转换格式、平台上传和排查故障的开发者。当前对应版本为 `4.4.1`。
...
2. 下载 `UMI-Data-Capture-Platform-4.4.1-Setup.exe`。
3. 右键安装包，选择“以管理员身份运行”。
...
- 浏览器崩溃或被强制关闭导致页面来不及通知后台时，后台会在约 120 秒无客户端心跳后自动退出。相机或夹爪被拔出只会更新对应设备状态，不会被误判为网页关闭，也不会主动结束后台。
...
模型闭合动画会根据实时归一化闭合度同步移动左右夹指。页面会根据各活动夹指的 STEP 几何边界计算安全行程，并使用平滑闭合曲线将端点映射到接近真实完全闭合的位置，减少夹指相互穿模；它是展示动画，不代替精确连杆运动学或机械干涉分析。

左右手使用各自的 IMU、相机特征和静止判定独立估计位姿与轨迹。一只手的运动、静止或跟踪质量不会驱动另一只手，也不会改变另一只手的轨迹；另一只手只作为同一场景中的可视参考。每只手在自身 IMU 和画面确认静止后独立执行零速度约束。

当前输出是相对于本次连接或重置时刻的估算位姿。4.4.1 增加更密集的特征补充、前后向 LK 光流一致性检查、旋转补偿视差和低速位移采样，使缓慢移动更容易形成连续轨迹，同时过滤纯旋转带来的假平移。IMU 用于方向、重力和静止约束，视觉光流按名义工作距离补足相对尺度；位置的左右、上下、前后映射到场景红、绿、蓝轴，并对明显上下或前后运动执行主轴串扰抑制。系统会保存重置时的画面特征；夹爪拿起后回到相近位置、方向和视野并保持静止时，会尝试把位置重定位为原点。单目视觉仍没有天然绝对尺度，页面稀疏点云也不是深度相机测得的真实距离；两只手不是经过外部标定的同一个世界坐标系。需要厘米级绝对定位时，还必须完成鱼眼内参、相机与 IMU 外参、硬件时间同步和外部基准标定。
...
- `closure`、`tracking_mode`、`visual_features`、`quality`、`stationary`、`origin_relocalized` 和 `cooperative_constraint`。其中 `cooperative_constraint` 为兼容旧数据结构保留，在 4.4.1 独立跟踪模式下固定为 `false`。
...
- 请确认使用 4.4.1 或更高版本；该版本将设备热拔插与网页生命周期分开处理。
- 正常拔出相机或夹爪后，后台应继续运行，页面只把对应槽位更新为未连接；重新插入后点击一次“重新扫描设备”。
- 若进程仍退出，检查后台末尾是否出现“最后一个网页已主动关闭”或“客户端心跳超时”。前者表示最后一个平台页已关闭，后者表示所有页面超过约 120 秒未发送心跳。
- 设备 SDK 或工作线程抛出的异常会被记录并隔离；提交问题时请保留退出前最后 50 行后台日志。
...
```text
../UMI-Data-Capture-Platform-4.4.1-Setup.exe
```
