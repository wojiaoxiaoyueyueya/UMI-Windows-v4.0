...
本文面向第一次接触本项目的使用者，也适用于需要换电脑部署、配置设备、录制数据、转换格式和排查故障的开发者。当前对应版本为 `4.3.2`。
...
2. 下载 `UMI-Data-Capture-Platform-4.3.2-Setup.exe`。
3. 右键安装包，选择“以管理员身份运行”。
...
页面提供轨迹长度、`1×~8×` 轨迹显示倍率、网格、模型、稀疏视觉点云、适配视图和重置原点控制。打开点云后，相机模型会显示向外展开的视锥，并将当前检测到的角点沿相机射线显示。鼠标左键旋转、滚轮缩放、右键平移；“适配视图”按当前模型和完整轨迹范围重新取景。显示倍率只放大页面中的运动幅度，不改变下方米制读数，也不改变录制文件。交换左右摄像头或左右手动夹爪后，页面与后端跟踪映射会同步更新。无硬件开发机可打开 `http://localhost:8080/?poseDemo=1` 检查模型、轨迹、相机视锥和闭合动画。
...
当前输出是相对于本次连接或重置时刻的估算位姿。低速移动时视觉光流按名义工作距离补足相对尺度，IMU 用于方向、重力和静止约束。系统会保存重置时的画面特征；夹爪拿起后回到相近位置、方向和视野并保持静止时，会尝试把位置重定位为原点。单目视觉仍没有天然绝对尺度，页面稀疏点云也不是深度相机测得的真实距离；两只手不是经过外部标定的同一个世界坐标系。需要厘米级绝对定位时，还必须完成鱼眼内参、相机与 IMU 外参、硬件时间同步和外部基准标定。
...
```text
../UMI-Data-Capture-Platform-4.3.2-Setup.exe
```
