// HttpServer.hpp - HTTP 服务器头文件（Linux 移植版，多摄像头）
// 基于 cpp-httplib 实现
...
private:
    ...
    // 多摄像头流开关
    mutable std::mutex streamFlagMutex_;
    std::map<std::string, std::map<std::string, bool>> streamFlags_;  // slot -> stream -> enabled
