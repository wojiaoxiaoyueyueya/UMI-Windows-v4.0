...
bool OrbbecCamera::startStreaming() {
    ...
    // 禁用帧聚合：有帧就立即回调，不等待所有帧类型齐全。
    // 之前的 ALL_TYPE_FRAME_REQUIRE 模式要求 color+depth+IR 全部到齐才触发回调，
    // 如果任何一路流（如 IR）不产出帧，所有流都会被阻塞 → 黑屏。
    ob_config_set_frame_aggregate_output_mode(config_, OB_FRAME_AGGREGATE_OUTPUT_DISABLE, &err);
    if (err) { ob_delete_error(err); err = nullptr; }
