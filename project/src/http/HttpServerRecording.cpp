...
#include <sys/wait.h>
#include <thread>
// Linux port: no windows.h needed
...
bool HttpServer::finalizeRecording(std::string sessionId,
    ...
    for (auto it = slots.begin(); it != slots.end(); ) {
        ...
        if (totalFrames == 0 && ss.gripperCount == 0 && ss.egCount == 0) {
            if (!ss.slotDir.empty()) {
                if (std::filesystem::exists(ss.slotDir)) {
                    std::filesystem::remove_all(ss.slotDir);
                }
            }
            it = slots.erase(it);
        } else {
...
bool HttpServer::startConversion(const std::string& sourceDir,
    ...
    convertThread_ = std::thread([this, sourceDir, sessions, task, outputDir, format]() {
        ...
        for (int i = 0; i < (int)sessions.size(); i++) {
            ...
            // Sanitize paths to prevent shell injection before passing to system()
            auto sanitizeForShell = [](const std::string& s) -> bool {
                return s.find_first_of("\"'`$\\;&|<>!") == std::string::npos;
            };
            if (!sanitizeForShell(script) || !sanitizeForShell(srcPath) || !sanitizeForShell(outPath) || !sanitizeForShell(progressPath) || (!task.empty() && !sanitizeForShell(task))) {
                std::lock_guard<std::mutex> lk(convertState_.mutex);
                convertState_.error = sessions[i] + " failed (invalid characters in path)";
                convertState_.converting = false;
                convertState_.currentStep = "Error";
                return;
            }

            // 使用 system() 调用 Python（Linux 上 python3 优先）
            std::string pythonBin = "python3";
            if (system("python --version >/dev/null 2>&1") == 0) pythonBin = "python";
            std::string cmd = pythonBin + " \"" + script + "\" \"" + srcPath + "\" \"" + outPath + "\"";
            ...
            std::lock_guard<std::mutex> lk(convertState_.mutex);
            if (exitCode != 0) {
                convertState_.error = sessions[i] + " failed (code " + std::to_string(exitCode) + ")";
                convertState_.converting = false;
                convertState_.currentStep = "Error";
                return;
            }
