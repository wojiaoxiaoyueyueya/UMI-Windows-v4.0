...
| 目录 | 当前版本 | 定位 | 维护状态 |
| --- | --- | --- | --- |
| [`UMI-Windows-V4`](UMI-Windows-V4/) | `4.4.1` | 当前 Windows 主版本，使用 UMI V4.0 手动夹爪协议 | 主动维护 |
...
- 左右手独立视觉与 IMU 相对位姿、可交互三维轨迹、防穿模闭合动画、相机视锥和稀疏视觉点云。
- 多设备统一会话时间基准、历史会话管理和后台转换队列。
- LeRobot、HDF5、RLDS 输出；无需填写任务指令，LeRobot 会按真实动作数据标记训练就绪状态。
- 平台上传：浏览并筛选本地转换结果，上传至本机配置的 MinIO，并在数据训练平台的指定任务和场景实例中自动登记待审核记录。
- Windows 一键安装，SDK 运行库和硬件驱动随安装包提供。
- 最后一个平台页面关闭后自动释放相机、串口和 CAN；设备拔插和 SDK 短时异常不会误关后台，异常残留进程由同一个启动入口自动检测和清理。
